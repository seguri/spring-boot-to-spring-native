...
kubectl create configmap precertification-service-loadtest --from-file main.py
...
if [ -z $(helm list | sed -n '/precertification-loadtestq/p') ]; then
  echo "Updating Helm repos"
  helm repo update
  echo "Installing precertification-loadtest"
  helm install precertification-loadtest deliveryhero/locust -f values.yaml --version 0.19.23
else
  echo "Upgrading precertification-loadtest"
  helm upgrade precertification-loadtest deliveryhero/locust -f values.yaml --version 0.19.23
fi
